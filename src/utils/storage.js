...
import { generateMarkdownSummary } from '@/domain/markdown/markdownRules'
...
export class DocumentStorage {
  ...
  async saveDocument(id, document) {
    try {
      ...
      if (!summary && cleanDoc.content) {
        summary = generateMarkdownSummary(cleanDoc.content, 150)
      }
  ...
  createMinimalDocument(document) {
    return {
      ...
      knowledgeType: String(document.knowledgeType || 'note'),
      knowledgeStatus: String(document.knowledgeStatus || 'draft'),
      confidence: String(document.confidence || 'medium'),
      aliases: Array.isArray(document.aliases) ? document.aliases.map(String) : [],
      sourceUrl: String(document.sourceUrl || ''),
      relatedIds: Array.isArray(document.relatedIds) ? document.relatedIds.map(String) : [],
      reviewedAt: document.reviewedAt ? String(document.reviewedAt) : null,
